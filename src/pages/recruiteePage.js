...
import H1 from '../components/P1_comp/h1';
import H3 from '../components/P1_comp/h3';
import H4 from '../components/P1_comp/h4';
import H5 from '../components/P1_comp/h5';
import H6 from '../components/P1_comp/h6';
import H7 from '../components/P1_comp/h7';
...
export const RecruiteePage = () => {
  ...
  useEffect(() => {
    Axios.get(`https://jobifybackend-pjf3.onrender.com/recruiteeRoute/recruiteePage/${state}`)
      .then((res) => setRecruitee(res.data))
      .catch((err) => alert(err));
  }, []); 
  const navigate=useNavigate(); 

  const handleClick=()=>{
    navigate(`/recruiteePage/profile/${state}`,{state:state});
  }

  const handleClickJobs=()=>{
    navigate(`/recruiteePage/jobs/${state}`,{state:state});
  }

  const handleClickHome=()=>{
    navigate(`recruiteePage/${state}`,{state:state});
  }

  const handleClickAbout=()=>{
    navigate(`recruiteePage/about/${state}`,{state:state})
  }
  ...
  return (
    (recruitee)?<div>
        ...
        <div className="container-fluid col-12">
          ...
          <div class="collapse navbar-collapse col-11" id="navbarSupportedContent">
            ...
            <ul class="col-lg-6 navbar-nav mb-2 mb-lg-0 col-sm-7 col-md-7 gap-sm-3">
              <li class="nav-item">
                <a  class="nav-link active fw-bold" aria-current="page" href="" onClick={handleClickHome}>
                  HOME
                </a>
              </li>
              <li class="nav-item">
                <a  class="nav-link fw-bold" href="" onClick={handleClickJobs}>
                  JOBS
                </a>
              </li>
              <li class="nav-item">
                <a className="nav-link fw-bold" href="" onClick={handleClickAbout}>ABOUT US</a>
              </li>
            </ul>
      ...
      <H1/>
        ...
        <H6/>
        <H7/>
      <p>Welcome:{recruitee.username}</p>
